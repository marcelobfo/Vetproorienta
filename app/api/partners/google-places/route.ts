...
export async function POST(req: NextRequest) {
  try {
    ...
    let baseState = (state || 'MG').trim();

    // 1. Google Places API Oficial (se houver chave configurada no ambiente)
      ...
      try {
        ...
        if (category && category !== 'all') {
          switch (category) {
            ...
            case 'especialista': textQuery = 'especialista veterinário dermatologia cardiologia'; break;
        ...
        console.warn('[Google Places API] Falha na chamada da API:', err.message);
      }
    }

    // 2. Geocodifica a cidade real do usuário caso não tenha GPS exato
    let targetLat = userLat;
    let targetLng = userLng;

    if ((!targetLat || !targetLng) && (baseCity || address)) {
      try {
        const queryText = address ? `${address}, ${baseCity || ''}, Brasil` : `${baseCity}, ${baseState}, Brasil`;
        const geoRes = await fetch(
          `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(queryText)}&format=json&limit=1`,
          {
            headers: { 'User-Agent': 'VetPro-Orienta/2.0 (suporte@vetpro.app)' }
          }
        );
        if (geoRes.ok) {
    ...
    // 3. Consulta de Alta Fidelidade com OpenStreetMap / Overpass API (Custo ZERO e Estabelecimentos 100% REAIS)
    if (targetLat && targetLng) {
      try {
        const overpassQuery = `
          [out:json][timeout:15];
          (
            node["amenity"="veterinary"](around:25000, ${targetLat}, ${targetLng});
            way["amenity"="veterinary"](around:25000, ${targetLat}, ${targetLng});
            node["shop"="pet"](around:25000, ${targetLat}, ${targetLng});
            way["shop"="pet"](around:25000, ${targetLat}, ${targetLng});
            node["healthcare"="veterinary"](around:25000, ${targetLat}, ${targetLng});
            way["healthcare"="veterinary"](around:25000, ${targetLat}, ${targetLng});
          );
          out center tags 40;
        `;
        ...
        if (overpassRes.ok) {
          ...
          for (const el of elements) {
            ...
            // Só aceita estabelecimentos com nome REAL mapeado (sem dados inventados)
            ...
            // Endereço real formatado a partir dos metadados geográficos
            ...
            // Telefone e contatos reais (NUNCA inventa DDD ou números)
        ...
        console.warn('Erro ao consultar Overpass API:', opErr);
      }
    }

    // 4. Fallback Complementar com Busca Direta no Nominatim por Termos Reais
    if (realPlaces.length < 5 && baseCity) {
      const searchQueries = [
        `veterinaria em ${baseCity} ${baseState}`,
        `pet shop em ${baseCity} ${baseState}`,
        `hospital veterinario em ${baseCity} ${baseState}`,
      ];

      for (const term of searchQueries) {
        try {
          const nomRes = await fetch(
            `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(term)}&format=json&addressdetails=1&limit=10`,
            ...
            if (Array.isArray(nomList)) {
              for (const item of nomList) {
                ...
                realPlaces.push({
                  ...
                  city: item.address?.city || item.address?.town || item.address?.municipality || baseCity,
                  state: item.address?.state || baseState,
                  latitude: pLat,
                  longitude: pLng,
                  rating: 4.8,
                  reviews_count: 18,
    ...
    // 5. Filtra por categoria e ordena por proximidade
